...
if [ "$ODOM_TYPE" = "csm" ]; then
  ...
  /usr/bin/gnome-terminal --tab -- bash -c "ros2 run robot_control_cpp gt_wrapper --ros-args -p use_sim_time:=true -p kobuki:=true; exec bash" &
  /usr/bin/gnome-terminal --tab -- bash -c "ros2 run robot_control_cpp scan_republisher --ros-args -r /scan_merged:=/scan_c -r /scan_merged_c:=/scan -p use_sim_time:=true -p kobuki:=true; exec bash" &
  ...
  /usr/bin/gnome-terminal --tab -- bash -c "ros2 launch genz_icp odometry.launch.py topic:=/pointcloud_topic min_consecutive_observations:=5 voxel_size:=0.1 kobuki:=true; exec bash" &
