...
# Bag playback — kill recording when bag finishes
...
if [ "$ODOM_TYPE" = "csm" ]; then
  ...
  /usr/bin/gnome-terminal --tab -- bash -c "ros2 run ros2_laser_scan_matcher laser_scan_matcher --ros-args -p scan_topic:=/scan -p wheel_odom_topic:=/odom -p use_point_to_line_distance:=1 -p use_sim_time:=true; exec bash" &
  ...
  /usr/bin/gnome-terminal --tab -- bash -c "ros2 launch genz_icp odometry.launch.py topic:=/pointcloud_topic use_sim_time:=true min_consecutive_observations:=8; exec bash" &
