...
Run [`kobukiSIM_setup.exe`](./resource/kobukiSIM_setup.exe), this will install the simulator and create shortcut on desktop.
...
1. Unzip [`uds_kobuki_sim.zip`](./resource/uds_kobuki_sim.zip) _outside of the ROS2 workspace folder_.
...
3. [Optional] Test SIM <-> ROS communication interface
    ...
    1. Change IP address - set the address of the Kobuki device
        1. uds_kobuki_ros run command in [`run.bat`](./deploy/run.bat) (Windows) / [`run.sh`](./deploy/run.sh) (Linux/WSL)
